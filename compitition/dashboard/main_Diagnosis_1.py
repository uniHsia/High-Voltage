...
def get_diagnosis_data():
    ...
    model_path = project_root / 'cross_attention_pd_model.pth'
    model = CrossAttentionPDDiagnosis(
        n_sensors=N_SENSORS,
        seq_len=N_PHASES,
        embed_dim=128,
    ...
    # 获取放电类型名称
    discharge_type_en = simulator.discharge_types[predicted_label]
    discharge_type_map = {
        'corona': '电晕放电',
        'internal': '内部放电',
        'surface': '沿面放电',
    ...
    discharge_type = discharge_type_map.get(discharge_type_en, discharge_type_en)
    confidence = probs[predicted_label] * 100

    # 根据置信度和放电类型判断严重程度
    if discharge_type_en == 'normal':
        severity = '正常'
    elif confidence > 90:
        severity = '严重'
    elif confidence > 70:
    ...
    result = {
        ...
        # 各放电类型概率（用于bar chart）
        'probabilities': {
            'types': [discharge_type_map.get(item, item) for item in simulator.discharge_types],
        ...
        # 相似案例（模拟）
        'similar_cases': [
            f'[{datetime.now().strftime("%Y-%m-%d")} {np.random.randint(10, 18):02d}:{np.random.randint(0, 59):02d}]',
            f'[{datetime.now().strftime("%Y-%m-%d")} {np.random.randint(10, 18):02d}:{np.random.randint(0, 59):02d}]'
        ]
    }
